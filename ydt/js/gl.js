...
var barMoveLength = 400  - 350;
//内容区域可滚动的距离，计算方式为内容区域的总高度减去内容区域本身的高度
var contentMoveLength ;
...
if (navigator.userAgent.indexOf("Firefox") < 0) {
    scrollContent.addEventListener("mousewheel", function (e) {
        handleMouseWheel(-e.wheelDelta /10, e);
    });
} else {
    scrollContent.addEventListener("DOMMouseScroll", function (e) {
        handleMouseWheel(e.detail /10, e);
    });
}
...
function scrollTot(top) {
   ...
   // console.log(contentMoveLength);
}

//将内容区域滚动某一相对距离
function scrollToRelative(relative) {
    scrollTot(scrollContent.scrollTop + relative);
}